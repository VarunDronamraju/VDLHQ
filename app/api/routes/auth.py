...
async def login(request: RoleLoginRequest):
    ...
    if request.role not in ["client", "ops"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid role. Must be 'client' or 'ops'.")
    ...
    payload = {"sub": user_id, "role": request.role, "exp": datetime.utcnow() + timedelta(days=7)}
